...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
	if (strcmp(cmdline, "mem") == 0 && cons->sht != 0) {
		...
		cmd_time(cons);
	} else if (strncmp(cmdline, "cdls", 4) == 0) {
		struct MEMMAN *memman = (struct MEMMAN *) MEMMAN_ADDR;
		char s[60];

		/* 初期化 */
		IDEDriver* driver = (IDEDriver *) memman_alloc_4k(memman, sizeof(IDEDriver));
		...
		/* 基本ボリューム記述子の読み込み */
		PrimaryVolumeDescriptor* pvd = (PrimaryVolumeDescriptor*) memman_alloc_4k(memman, sizeof(PrimaryVolumeDescriptor));
		...
		byte* buffer = (byte *) memman_alloc_4k(memman, size);
		...
		memman_free_4k(memman, (int) buffer, size);
	} else if (cmdline[0] != 0) {
		if (cmd_app(cons, fat, cmdline) == 0) {
			/* コマンドではなく、アプリでもなく、さらに空行でもない */
			cons_putstr0(cons, "無効なコマンドです。\n\n");
		}
	}
}
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		hrb_api_osselect(fh->buf, (int *) memman_alloc_4k(memman, 512 * 1024));
 		...
 		struct MOUSE_INFO *info = (struct MOUSE_INFO *) (ecx + ds_base);
 		int tx = 0, ty = 0;
 		if (ebx != 0) {
 			sht = (struct SHEET *) (ebx & 0xfffffffe);
 			tx = sht->vx0;
 		...
 		info->x = moinfo.x - tx;
 		info->y = moinfo.y - ty;
 		info->button = moinfo.button;
 		info->state  = moinfo.state;
 	} else if (edx == 0x2000) {
		    unsigned char s[24], t[7];
		    readrtc(t);
		    sprintf(s, "%02X%02X.%02X.%02X %02X:%02X:%02X\n", t[6], t[5], t[4], t[3], t[2], t[1], t[0]);
